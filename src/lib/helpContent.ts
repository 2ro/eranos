 ...
 *
 * Any page can call `getFAQCategories(appName)` or `getFAQItems(appName)`
 * to render a full FAQ or a filtered subset (e.g. only "payments"
 * questions on a wallet settings page). Internally these resolve strings
 * through `i18n.t()`, so callers must trigger a re-render when the active
 * language changes — `HelpFAQSection` and `HelpTip` do this by depending
 * on `i18n.language` via `useTranslation()`.
 *
...
export function getFAQItems(appName: string, categoryId?: string): FAQItem[] {
  const cats = categoryId
    ? FAQ_STRUCTURE.filter((c) => c.id === categoryId)
    : FAQ_STRUCTURE;
  return cats.flatMap((c) => c.items).map((i) => resolveItem(i.id, appName));
}
